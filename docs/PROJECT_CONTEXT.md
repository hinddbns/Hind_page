...
| `/mot-de-passe-oublie` | `mot-de-passe-oublie/page.tsx` | Password-reset request: `supabase.auth.resetPasswordForEmail(email)`, then redirects to `/reinitialiser-mot-de-passe?email=…`. Never reveals whether the address has an account (errors swallowed, same redirect always). | Fully built. |
| `/reinitialiser-mot-de-passe` | `reinitialiser-mot-de-passe/page.tsx` | OTP-based recovery: enter the 6-digit code from the email → `supabase.auth.verifyOtp({ email, token, type: "recovery" })` → choose new password → `supabase.auth.updateUser({ password })`. Steps `otp`/`password`/`done`, with expiry countdown + resend cooldown (mirrors `/verification-email`). No clickable reset link. Requires the Supabase "Reset Password" email template to render `{{ .Token }}`. | Fully built. |
...
| `CourseCard.tsx` | Course preview card: demo-video badge or title-initial placeholder, title, clamped description, price, CTA link. | Both workspace pages, `/cours` | `course: {...}`, `ctaLabel`, `demoLabel` |
...
| Component | Does | Used in | Key props |
|---|---|---|---|
| `ConfirmActionForm.tsx` | Wraps **any** `ActionState`-returning Server Action in a zero-visible-field form; the submit button optionally gates on an accessible `ConfirmDialog` (not `window.confirm` — focus trap, `Escape`, `role="dialog"`) before submitting, shows pending state via `useFormStatus` (through the shared `FormSubmitButton`), shows `state.error` inline, and reports success via the global toast (`useToastActionState`, with an optional `successMessage` prop) rather than an inline banner — this specifically survives the mutating button being unmounted by the very re-render its own success causes (e.g. approve/reject, promote/demote). **The** pattern for every destructive/state-changing admin button. | Everywhere in `/admin` (delete, publish toggle, approve/reject, promote/demote, questionnaire toggle) | `action`, `confirmMessage?`, `successMessage?`, `label`, `pendingLabel`, `className` |
| `CourseForm.tsx` | Create/edit course: title, slug (create-only), price, audience selector, description, demo video (file or URL). Self-managed open/close state (a button, not `<details>`, so it survives the server-driven re-render after a successful save). Uses `useId()` for label association. | `admin/cours`, `admin/cours/[id]` | `action`, `mode: "create" \| "edit"`, `initial?`, `defaultOpen?` |
